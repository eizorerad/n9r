...
> **📖 For a detailed explanation of the AI Scan architecture, see [AI_SCAN_EXPLAINED.md](./AI_SCAN_EXPLAINED.md)**

### Triple-Track Parallel Analysis

When you trigger an analysis, **three independent pipelines run in parallel**:
...
└─────────────────────────────────────────────────────────────────┘
    ...
    │ Static   │      │Embeddings│     │ AI Scan  │
    │ Analysis │      │Generation│     │ (LLMs)   │
    └──────────┘      └──────────┘     └──────────┘
         │                 │                 │
         │                 │                 │
    All three run simultaneously (~50% faster!)
```

### The Resurrection Pipeline (Track 1: Static Analysis)
...
### The Séance Pipeline (Track 2: Semantic Analysis)
...
### The AI Scan Pipeline (Track 3: Multi-Model LLM Analysis)

```
┌─────────────┐     ┌─────────────┐     ┌─────────────────────────────┐
│  📋 Generate│────▶│  🤖 Parallel│────▶│  🔀 Merge &  │────▶│ 💾 Cache│
│  Repo View  │     │  LLM Scan   │     │  Deduplicate │     │ Results │
└─────────────┘     └─────────────┘     └──────────────┘     └─────────┘
                           │
              ┌────────────┴────────────┐
              ▼                         ▼
         ...
         │ 3 Pro   │              │Sonnet4.5│
         └─────────┘              └─────────┘
         Both models scan simultaneously!
         
    → Detects: Security issues, bugs, code smells
    → Returns: Structured JSON with actionable fixes
    → Consensus: Issues found by both models = higher confidence
```
